...
# Student Info
    ...
    Name: Hilde Younce
    Ned ID: ksg8xy
    URL of this file on GitHub: https://github.com/hyounce/DS5100-ksg8xy/blob/main/lessons/M10/hw10.Rmd
...
cut_sum <- c(0.4, 0.7, 0.9, 1.0)
quota <- 225000
amt <- cut * quota
payout = amt * payout_pct
payout_sum <- c(payout[1], payout[1]+payout[2], payout[1]+payout[2]+payout[3], payout[1]+payout[2]+payout[3]+payout[4])
amt_sum <- c(amt[1], amt[1]+amt[2], amt[1]+amt[2]+amt[3], amt[1]+amt[2]+amt[3]+amt[4])
sales = data.frame(cut, payout_pct, cut_sum, amt, payout, payout_sum, amt_sum)
print(sales)
```
...
```{r}
amount_earned <- function(frac){
  if (frac <= 0.4) row = 1
  else if (frac <= 0.7) row = 2
  else if (frac <= 0.9) row = 3
  else row = 4
  
  if (row > 1){
    a = payout_sum[row-1]
    b = quota * (frac - cut_sum[row-1]) * payout_pct[row]
  }
  else{
    a = quota * frac * payout_pct[row]
    b = 0
  }
  return (a + b)
}
...
```{r}
percent <- seq(0, 1, 0.1)
payouts <- sapply(percent, amount_earned)
data = data.frame(percent, payouts)
print(data)
```
...
defined above.
```{r}
plot(amt_sum, payout_sum)
```

```{r}
plot(amt_sum, payout_sum, type='l')
```
# Task 5 
